...
class PH470MGUI:
    ...
    def create_control_panel(self):
        ...
        text_replacer_btn = tk.Button(
        dorks_frame, 
        text="Text Replacer", 
        command=self.text_replacer, 
        bg="#4CAF50", 
        fg="white", 
        width=20, 
        font=("Helvetica", 9, "bold")
        )
        text_replacer_btn.grid(row=4, column=0, sticky=tk.W, padx=10, pady=5)
        ToolTip(text_replacer_btn, "Replace any specified text within the loaded dork queries.")
    ...
    def text_replacer(self):
        # Prompt for the text to find
        find_text = simpledialog.askstring("Find Text", "Enter the text you want to replace:", parent=self.root)
        if not find_text:
            messagebox.showerror("Error", "Find text cannot be empty.")
            return

        # Prompt for the replacement text
        replace_text = simpledialog.askstring("Replace With", "Enter the replacement text:", parent=self.root)
        if replace_text is None:  # Allow empty replacement (to delete occurrences)
            return

        # Get current content in the text area
        current_content = self.dorks_text.get("1.0", tk.END).strip()
        if not current_content:
            messagebox.showerror("Error", "No content loaded to perform replacement.")
            return
        ...
        self.dorks_text.insert(tk.END, updated_content)

        # Inform the user about the replacement
        messagebox.showinfo("Replacement Complete", f"Replaced all occurrences of '{find_text}' with '{replace_text}'.")
